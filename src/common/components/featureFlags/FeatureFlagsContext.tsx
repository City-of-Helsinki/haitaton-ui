...
export function FeatureFlagsProvider({ children }: FeatureFlagsProviderProps) {
  const value: FeatureFlagsContextProps = useMemo(
    () => ({
      publicHankkeet: String(window._env_.REACT_APP_FEATURE_PUBLIC_HANKKEET || '') === '1',
      hanke: String(window._env_.REACT_APP_FEATURE_HANKE || '') === '1',
      cableReportPaperDecision:
        String(window._env_.REACT_APP_FEATURE_CABLE_REPORT_PAPER_DECISION || '') === '1',
      informationRequest: String(window._env_.REACT_APP_FEATURE_INFORMATION_REQUEST || '') === '1',
    }),
    [],
  );
