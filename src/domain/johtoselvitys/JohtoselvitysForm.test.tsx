...
import { renderJohtoselvitys, goToContactsStep } from './johtoselvitysTestHelpers';
...
async function fillContactsInformation(user: UserEvent) {
  ...
  await selectComboboxOption(user, { labelRegex: /tyyppi/i, optionRegex: /yritys/i, index: 1 });
...
async function fillCustomerInformation(user: UserEvent) {
  ...
  if ((typeSelect as HTMLInputElement).value === '') {
    await selectComboboxOption(user, { labelRegex: /tyyppi/i, optionRegex: /yritys/i, index: 0 });
  }
...
// (goToContactsStep moved to johtoselvitysTestHelpers)
...
test('Should be able to send application', async () => {
  ...
  const { user } = renderJohtoselvitys({ hankeData, application: hakemus });
...
test('Validation error is shown if no work is about checkbox is selected', async () => {
  const { user } = renderJohtoselvitys();
...
test('Should be able to upload attachments', async () => {
  ...
  // Use central helper to upload test attachments (it mocks API and returns uploaded file names)
  const uploaded = await uploadTestAttachments(user, {
    liikenne: new File(['test-a'], 'test-file-a.pdf', { type: 'application/pdf' }),
    muu: new File(['test-b'], 'test-file-b.pdf', { type: 'application/pdf' }),
  });
  expect(uploaded).toEqual(['test-file-a.pdf', 'test-file-b.pdf']);
...
test('Should list existing attachments in the attachments page and in summary page', async () => {
  ...
  const { user } = renderJohtoselvitys({ application: enriched, props: { initialStep: 3 } });
  await screen.findByText(/Vaihe 4\/5: Liitteet/);
...
test('Summary should show attachments and they are downloadable', async () => {
  ...
  const { user } = renderJohtoselvitys({ application: testApplication, props: { initialStep: 4 } });
...
test('Should be able to create new user and new user is added to dropdown', async () => {
  ...
  const { user } = renderJohtoselvitys({ application: testApplication, props: { initialStep: 2 } });
