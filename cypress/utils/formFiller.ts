...
export const fillForm2 = (hankeData: HankeDataDraft) => {
  if (hankeData.omistaja) {
    cy.get('input[data-testid=omistaja.nimi]').type(hankeData.omistaja.nimi);
    cy.get('input[data-testid=omistaja.email]').type(hankeData.omistaja.email);
    cy.get('input[data-testid=omistaja.puhelinnumero]').type(hankeData.omistaja.puhelinnumero);
  }
};
